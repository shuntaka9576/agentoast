...
fn detect_agent_status(
    ...
    match agent_type {
        ...
        "opencode" => detect_opencode_status(db_conn, pane_id),
        _ => {
            log::debug!(
...
// ──────────────────────────────────────────────────────────
// OpenCode-specific agent status detection
// ──────────────────────────────────────────────────────────
...
fn check_opencode_pane_content(pane_id: &str) -> OpencodePaneContentInfo {
    ...
    let tmux_path = match find_tmux() {
        Some(p) => p,
        None => {
            log::debug!("check_opencode_pane_content: tmux not found");
            return default;
        }
    };
    ...
    let Some(output) = output else {
        log::debug!(
            "check_opencode_pane_content({}): capture-pane exec failed",
            pane_id
        );
        return default;
    };
    if !output.status.success() {
        log::debug!(
            "check_opencode_pane_content({}): capture-pane exit={} stderr={}",
            pane_id,
            output.status,
            String::from_utf8_lossy(&output.stderr)
        );
